...
def get_sol_start_time(sol_data, job_id):
    return next(
        (item["StartTime"] for item in sol_data["Jobs"] if item["JobId"] == job_id), -1
    )


def get_sol_machine_id(sol_data, job_id):
    return next(
        (item["MachineId"] for item in sol_data["Jobs"] if item["JobId"] == job_id), -1
    )
...
def check_no_job_overlap(instance_data, sol_data):
    for j1, j2 in combinations(instance_data["Jobs"], 2):
        ...
        j1_m, j2_m = (
            get_sol_machine_id(sol_data, j1_id),
            get_sol_machine_id(sol_data, j2_id),
        )

        if j1_id != j2_id and j1_m == j2_m:
            j1_start, j2_start = (
                get_sol_start_time(sol_data, j1_id),
                get_sol_start_time(sol_data, j2_id),
            )
            j1_end, j2_end = (
                j1_start + j1["ProcessingTime"],
...
def check_job_precedences(instance_data, sol_data):
    for j in instance_data["Jobs"]:
        ...
        for p in j["PrecedenceJobIds"]:
            end_p = (
                get_sol_start_time(sol_data, p)
                + get_inst_job(instance_data, p)["ProcessingTime"]
            )
...
def check_setup_times(instance_data, sol_data):
    for m in instance_data["Machines"]:
        scheduled_jobs = sorted(
            [j for j in sol_data["Jobs"] if j["MachineId"] == m["Id"]],
            ...
            events.append(
                {
                    "ResourceId": r_id,
                    "Time": p["Start"],
                    "Type": ResourceEvent.PERIOD_START,
                    "Capacity": p["Capacity"],
                }
            )
        events.append(
